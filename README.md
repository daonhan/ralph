...
`/model` stored; its "(default)" entry stores no model) → `claude-opus-5[1m]`,
Ralph's own default. Ralph always passes `--model`: the sandbox image's CLI is
frozen at image build time, so letting it pick its own built-in default would
silently run an older model than the host shows. Isolated Codex defaults to
...
| `RALPH_MODEL`            | Claude `claude-opus-5[1m]`; isolated Codex uses `gpt-5.6-sol`/high | Model override for the selected agent. Claude falls back to the `model` in host `~/.claude/settings.json`, then Ralph's own default (never the sandbox CLI's frozen default).                                                                                                               |
