...
- [ ] After the fix lands, the previously-required manual recovery (`docker ps --filter ancestor=docker.io/daonhan/ralph-sandbox:latest` + `docker kill <id>`) is no longer needed for the post-`result` self-deadlock scenario — ralph self-recovers within the grace period.
- [ ] The 0.6.1 release notes (CHANGELOG entry generated by release-please from the `fix(core): …` commit) call out the manual `docker kill` runbook for operators still on `@daonhan/ralph-core` ≤ 0.6.0.
