...
Work through the plan/PRD tasks. When none is left to pick up, do no other work on that iteration and end the final message with `<promise>NO MORE TASKS</promise>` on a line of its own. Anywhere else, never mention it in prose and never in a message that reports completed work — a mention does not end the run, and the next iteration will emit it.
