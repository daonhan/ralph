...
Beside it each run keeps an append-only event log (`<run>.jsonl`; the newest 20 that weren't
refused are kept, plus the newest refused one) that a supervisor can read to tell whether the run
is still going, how it ended, and how long its current stage and agent have been silent. Only one
run per workspace: a launch exits `75` while another run is live or a killed run's container is
still running. A run whose last iteration failed exits `1`; `no-more-tasks` and the iteration cap
exit `0`, and `Ctrl+C` exits `130` (SIGTERM `143`). Schema: [ARCHITECTURE.md](https://github.com/daonhan/ralph/blob/main/docs/ARCHITECTURE.md#run-event-log).
